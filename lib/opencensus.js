...
var Stackdriver = (function (){
    function Stackdriver(config) {
        if (config.opencensus.stackdriver === "undefined") {
            // Unable to proceed
            throw new Error("GCP Project ID is required for Stackdriver");
        }
        this.projectId = config.opencensus.stackdriver.projectId;
    }
...
var Prometheus = (function (){
    function Prometheus(config) {
        this.port = ((port) => port !== "undefined" ? port : 9464)(config.opencensus.prometheus.port);
    }
...
var OpenCensus = (function() {
    ...
    function OpenCensus(config, events, logger) {
        ...
        // });
        this.exporters.stackdriver = new stackdriver.StackdriverStatsExporter(new Stackdriver(config).get());
        ...
        //     startServer: true
        // });
        this.exporters.prometheus = new prometheus.PrometheusStatsExporter(new Prometheus(config).get());
